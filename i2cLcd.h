 ...
 */
#define LCD__SLAVE_ADDRESS(lcd) (0x40 | ((lcd)->A0A1A2 << 1))

/**
 * @brief Generate the cursor options bits.
 * @param lcd: Pointer to the LCD handle
 */
#define LCD__CURSOR_OPTIONS(lcd) (((lcd)->cursor << 1) | (lcd)->cursorBlink)
 ...
 */
#define LCD__GENERATE_COMMAND_SIGNALS(enBit, rwBit, rsBit) (0x08 | (enBit << 2) | (rwBit << 1) | rsBit)
